...
threshold unmet (show ADVISORY). Earlier proposal of a non-Gemini Model B is OFF (Model B is also Gemini;
hackathon mandates Gemini).

**Consequence for an earlier research finding (flaw #2):** since both models are Gemini and Gemini 3.x deprecates
...
- *generate_content*: still supported, but Google now leads its own structured-output + function-calling
  guides with Interactions; would mean owning thought-signature replay ourselves (§10), which is more fragile.
...
## TD-009: Operating model: builder agents build, an orchestrator verifies
**Date:** 2026-08-20 · **Status:** LOCKED (user directive)
Implementation is delegated to builder agents (Agent tool, builder role); the orchestrator main session
orchestrates, writes the brief, and INDEPENDENTLY verifies (re-runs `make check`, reads load-bearing files)
before committing + tagging. Rationale: save orchestrator tokens while keeping the orchestrator as
architect/verifier. Honors the spec's intent (builder agents implement, the orchestrator does
architecture/verification) and logs the real model used per phase.
Builder agents must NOT commit/tag/push. The orchestrator does that after verification.
...
built by builder agents with the orchestrator verifying (TD-009). Two loops:
