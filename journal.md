...
## Current Status
SELF-IMPROVING LOOP (Phases A-C) COMPLETE + VERIFIED (2026-08-22), built by builder agents, the orchestrator
independently re-ran `make check` + read the load-bearing files + committed on `dev` (TD-009). Decision: TD-014.
  ...
  - Setup: the project's internal working agreement (standing order "builder agents build, the orchestrator
    orchestrates+verifies+commits" = TD-009; "keep ARCHITECTURE.md true in the same change"), `ARCHITECTURE.md`
    (backend map, kept current each phase), and an internal phase plan (kept locally, not published).
    Full suite: 414 py + 9 vitest green.
...
PHASE 21 (four builder-agent workstreams built, the orchestrator verified+integrated+deployed; decisions TD-010..013):
...
LIVE DEPLOY (the orchestrator ran infra/ 00→50 as the project owner via gcloud auth; guardrail confirmed active gcloud project was
...
FRONTEND LIVE (public, 2026-08-21): https://chartpilot-frontend-zkhsg5lcca-el.a.run.app (Cloud Run service
  ...
  Verified: / → 200 <title>ChartPilot</title>, /patients/MRN-48213 → 200. A builder agent containerized (next.config
  output:standalone + frontend/Dockerfile node:22-slim pinned, non-root node user, $PORT); the orchestrator deployed via
...
PHASE 19 (a builder agent built, the orchestrator independently verified): the deployed Cloud Run worker now REALLY runs the pipeline.
...
VERIFY (the orchestrator, independent; 2026-08-21): make check PHASE=19 exit 0, 317 passed. test_composition.py 11/11
...
INFRA (Part B, orchestrator-authored, NOT run): infra/{_config,00_enable_apis,10_service_accounts,20_firestore,
...
PART A DELIVERED (a builder agent built, the orchestrator independently verified):
...
VERIFY (the orchestrator, independent; 2026-08-21): make check PHASE=18 exit 0, 306 passed. Docker §76A.2: build exit 0;
...
LIVE MEASUREMENTS (the orchestrator, 2026-08-20, our .env key):
...
Next action: PHASE 17: latency + eval. Instrument run_patient per-stage timing; scripts/measure_latency.py
  (single-patient ≤90s p50/worst, LIVE, the orchestrator runs); precomputed multi-patient run artifact (§50, hermetic);
  scripts/measure_model_b_live.py (§22.3 Set M live catch/false-accept/false-reject, the orchestrator runs); EVALUATION.md.
...
Step: tests/adversarial/: §53 injection invariant (byte-equality, no exception clause: benign vs
  ...
  PHASE=15` exit 0, 278 tests. the orchestrator verified the injection test is genuine byte-equality.
...
Step: app/pipeline/runner.py chains all phases (FETCHING→…→PERSISTED, fail-closed). 5 demo patients A-E
  ...
  PHASE=14` exit 0, 255 tests. the orchestrator verified runner.py + re-ran make check.
MILESTONE: FIRST LIVE END-TO-END (the orchestrator ran real Gemini A=3.7-flash/B=3.5-flash on Patient A):
...
Step: frontend/ Next.js 15.5.23 + React 19.2.8 + TS strict, CSS design tokens (dark M3-inspired), typed mock
  ...
  GATE: `pnpm run build` exit 0 (5 routes SSG) + a11y pass. evidence/phase_13.txt. the orchestrator re-ran build+a11y,
...
Step: app/storage/ (PatientSummary §44 compact; chunk_documents ≤400 [Firestore 500/commit limit];
  ...
  `make check PHASE=12` exit 0, 243 tests. google-cloud-firestore==2.28.1. the orchestrator verified two_phase.py + rules.
GCP research DONE → research/gcp-notes.md (Firestore 500/commit; asia-south1 has all infra; Gemini Dev API
  is global so region caveat N/A). Playbook pointer recorded internally.
...
Step: app/tasks/ (RunTask, ExecutionBudget/Counters, Checkpoint §45; CheckpointStore + TaskQueue +
  ...
  exit 0, 227 tests. the orchestrator verified orchestrator.py. GCP project = chartpilot-agentic (#124294464754), billing on.
...
Step: app/gate/ (claim_gate finalize_claim_verdict §42; patient_state derive_patient_status +
  assert_state_invariants §43). `make check PHASE=10` exit 0, 211 tests. the orchestrator verified both (deterministic
...
Step: app/agent/ (protocol, models=Claim schema §40, claims parse fail-closed, toolcall §10 strict-match,
  ...
  pyyaml added. the orchestrator verified models.py/toolcall.py/check script; re-ran make check.
Next action: PHASE 8: deterministic citation checker (Gates 1-4: source retrieval, content hash, verbatim
  ...
  Then Phase 9 (Model B), Phase 10 (safety gate). Continue via builder agents; the orchestrator verifies.
...
Step: app/evidence/ (errors, models, hashing, throttle, budget, openfda, pubmed, guideline_pack, snapshot)
  ...
  `make check PHASE=06` exit 0, 128 tests. the orchestrator verified openfda.py (SPL policy flags ambiguity, no arbitrary
...
Last successful test: `make check PHASE=03` → exit 0, 20 tests (the orchestrator re-ran + read client.py/transport.py).
Next action: PHASE 4: clinical data normalizer. A builder agent builds the Observation normalizer per SPEC §26/§27/§28
  ...
  MedicationRequest≠adherence §31). the orchestrator designs the normalized model + verifies.
...
Builder agents BUILD (implementation); the orchestrator (main session) ORCHESTRATES + independently VERIFIES
(re-runs make check, reads key files) before committing/tagging. Saves tokens. Records real model used.
Phase 2 built by a builder agent (general-purpose builder role); verified + committed by the orchestrator. See TD-009.

## Continuation quick-ref (if session interrupted)
Read the internal spec + this journal + the internal plan doc (kept locally, not published).
`git tag -l 'phase-*'` → highest is phase-01. Branch `dev`.
...
then the internal plan doc (once it exists), then the internal open-questions doc and TECHNICAL_DECISIONS.md.
We are in Phase 0. Do NOT write application code until Phase 0 questions are resolved and the plan is finalized.
...
- TD-002 (2026-08-20): New, fully isolated Google Cloud project. Existing projects must not be
  touched. Active gcloud default is currently `iatronix-med-search-v1` (a PRODUCTION project), a
...
- Model routing reality (§4): this session runs the orchestrator-tier model (planning tier, correct). I cannot
  self-switch the main-loop model to the builder-tier model mid-session; that is a user action (/model) or done
  via subagents. Will record the actual model used per phase rather than pretend routing.

## Mistakes / Corrections
- 2026-08-20 (Phase 17): a builder agent renamed `api_key` vars in scripts/measure_latency.py +
  ...
  pattern (harness security-review flagged it). FIX (the orchestrator): (1) improved secret_scan.sh: the generic
...
## Open Questions
See the internal open-questions doc (kept locally). Answered: Iatronix reuse (yes, with disclosure); isolated GCP project (yes).
...
- Spec's global "no temperature/top_p/top_k" grep gate (§65.4 Phase 7) conflicts with using another
  vendor's model as Model B where temperature is the correct determinism lever. Needs reconciliation in the plan.
